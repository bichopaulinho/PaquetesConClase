...
RastriginFunction::RastriginFunction(void) : ObjectiveFunction()
{
   numberOfVariables = 2;
...
double RastriginFunction::getEvaluation(Vector<double> argument)
{
...
Vector<double> RastriginFunction::getGradient(Vector<double> argument)
{
...
/// This method returns the Rosenbrock's analytical Hessian Matrix 
/// for a given argument.
///
/// @param argument: Point at which the Hessian is to be computed.

Matrix<double> RastriginFunction::getHessian(Vector<double> argument)
{
...
Matrix<double> RastriginFunction::getInverseHessian(Vector<double> argument)
{
   ...
   for(int i = 0; i < numberOfVariables; i++)
   {
      for(int j = 0; j < numberOfVariables; j++)
      {
         ...
         {
            inverseHessian[i][j] 
            = 1.0/(2.0 + 10.0*cos(2.0*pi*argument[i])*4.0*pow(pi,2));
         }
         else
         {
